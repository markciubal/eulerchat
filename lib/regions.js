...
/**
 * How many subjects the census counts together: every combination of up to
 * three of what each person holds.
 *
 * Three because that is what the circles can draw — no arrangement of them
 * realises all fifteen regions of a 4-set Venn diagram, a hard geometric fact
 * rather than a rendering budget — and because the census is the sum, over
 * everybody, of every combination of what they hold up to this size. At three
 * that is cubic in a subscription: 5,488 regions for somebody at the limit of
 * 32. At ten it would be 107 million, for one person.
 *
 * Not how many a room may combine: see `ROOM_ARITY`.
 */
export const MAX_ARITY = 3;

/**
 * How many subjects one room may combine.
 *
 * The map is not circles any more. It is grown from who holds what, draws a
 * region of any size, and draws exactly the ones somebody stands in; with ten
 * subjects on it, somebody holding six of them stands in a room of six. So a
 * room may combine as many as the map can show, and its population is
 * counted when it is asked for, from the people holding every one of its
 * subjects, rather than kept in the census.
 */
...
const average = (list, of) => {
  if (!list.length) return 0;
  let sum = 0;
  for (const item of list) sum += of(item);
 ...
 */
export function neighbourhood(counts, subscription, limit = MAX_ARITY, index, options = {}) {
  const { population, adjacency, popular } = index ?? buildIndex(counts);
  // 0 goes deeper into what they already hold; 1 reaches for the far side of
  // whatever they are connected to. Suggesting purely by shared membership is
  // a dial stuck at zero: it always proposes the nearest neighbour, which is
  // the one they were most likely to find unaided.
  const novelty = Math.max(0, Math.min(1, options.novelty ?? 0));
  const distance = options.distance ?? null;
  // Which subjects may be suggested at all. What somebody holds is theirs to
  // see whatever this says; it only narrows what is put next to it. A group
  // uses it to keep a member's view inside the group rather than filling it
  // with the busiest rooms of the open world, or of somebody else's group.
  const allowed = options.allowed ?? (() => true);
  const rank = (a, b) =>
    (population.get(b) ?? 0) - (population.get(a) ?? 0) || a.localeCompare(b);

  // Which of what they hold is shown when it is more than fits. `first` puts
  // some ahead of the rest whatever their size — a group ahead of the world
  // outside it — and after that the busiest go first. By size alone a group
  // always lost: its rooms hold the handful of people in it, so somebody
  // holding a few busy interests outside joined a group, or joined something
  // inside one, and the map carried on showing only the interests outside.
  const first = options.first ?? (() => 0);
  const mine = canonical(subscription)
    .filter((s) => population.has(s))
    .sort((a, b) => first(a) - first(b) || rank(a, b));
  ...
  while (chosen.length < limit) {
    if (!chosen.length) {
      ...
      const first = popular.find(allowed);
    ...
    const pool = new Set();
    for (const c of chosen) for (const n of adjacency.get(c) ?? []) if (allowed(n)) pool.add(n);
    for (const c of chosen) pool.delete(c);

    // Affinity is scored against the strongest candidate rather than in
    // absolute terms, so that novelty trades against it on the same scale
    // whatever the size of the world.
    let strongest = 0;
    ...
    for (const s of pool) {
      const near = strongest > 0 ? shared.get(s) / strongest : 0;
      // A subject reachable through people but far away in the taxonomy is
      // what a new community looks like from the inside.
      const far = distance ? Math.min(1, average(chosen, (c) => distance(s, c))) : 0;
      const score = (1 - novelty) * near + novelty * far * (shared.get(s) > 0 ? 1 : 0.25);
    ...
    best ??= popular.find((s) => !chosen.includes(s) && allowed(s)) ?? null;
    if (best === null) break;
    chosen.push(best);
  }
