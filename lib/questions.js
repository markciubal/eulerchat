...
/**
 * Four or more, `{all}` every one of them. A room can hold up to ten now, and
 * a question that named three of them and left the rest out would be about a
 * different, smaller room than the one it was asked in.
 */
const MANY = [
  'What brought you to {all}, all at once?',
  'Is there one thing that ties {all} together?',
  'Of {all}, which two sit closest together for you?',
  'Where do {all} all meet in something you have done or read?',
  'Which of {all} would you explain first to somebody new to the rest?',
];
...
export function ask(subjects, { random = Math.random } = {}) {
  ...
  if (sorted.length > 3) return fill(pick(MANY, random), { all: listOf(sorted) });
  const [a, b, c] = sorted;
