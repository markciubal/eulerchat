...
export function atlas(zoneCounts, options = {}) {
  const {
    ...
    smooth = 3,
    extent = 1000,
    // Where subjects sit before membership has any say — see lib/taxonomy.js.
    anchors = null,
    // Let a Physarum model decide which way the ground runs between them.
    mold = false,
    // Subjects that wrap all the others, held in every region: a group's own
    // conversation. Laid out without, and drawn round everything.
    frames = [],
  } = options;
  ...
  // A frame is held by every region, so it says nothing about where anything
  // goes — and left in, it made every region share a subject with every
  // other, pulled them all together, and tore apart the subjects that should
  // have held together. Inside a group that drew the same people differently
  // from how they are drawn outside it. So the layout is worked out without
  // the frames, exactly as it would be for the same people with no group
  // round them, and the frames are put back to be drawn: a subject every
  // region holds has the whole of the ground for its territory, whatever the
  // layout did. A region of nothing but frames — somebody in the group who
  // has joined nothing else yet — is kept, and sits near the middle.
  const wrap = new Set(frames.filter((f) => regions.every((r) => r.subjects.includes(f))));
  const laidOut = subjects.filter((s) => !wrap.has(s));
  const full = regions.map((r) => r.subjects);

  const lay = (framed) => {
    if (framed) for (const r of regions) r.subjects = r.subjects.filter((s) => !wrap.has(s));
    const at = placeSubjects(framed ? laidOut : subjects, regions, extent, anchors);
    seedRegions(regions, at, extent, fill);

    // The mould weaves the channels between subjects; the quotas below still
    // decide how much ground each zone gets, so the shape changes and the
    // areas do not.
    const woven = mold ? weaveBetween(framed ? laidOut : subjects, regions, at, extent, grid, mold) : null;

    let owner = grow(regions, grid, fill, extent, woven);
    // One relaxation pass: re-seed each zone at the middle of the ground it
    // won and grow again, which settles the patches into rounder shapes.
    reseed(regions, owner, grid, extent);
    owner = grow(regions, grid, fill, extent, woven);
    closePockets(owner, grid, regions);
    regions.forEach((r, i) => (r.subjects = full[i]));
    return { owner, woven };
  };

  let { owner, woven } = lay(wrap.size > 0 && laidOut.length > 0);

  // The frame is the one promise a group's picture makes: everything in the
  // group inside one line. Laid out without it, rooms that share nothing can
  // drift apart and leave the ground in two pieces, and the line round it
  // with them — rarely, about one group in a hundred. When that happens the
  // group is laid out again with its frame as an ordinary subject, which every
  // region shares and which therefore holds them all together.
  if (wrap.size > 0 && laidOut.length > 0 && !whole(owner, grid)) ({ owner, woven } = lay(false));
  ...
  // Each zone's own ground: where to hang its label, and its outline.
  //
  // The outline is new, and it is what lets a room be drawn as a room rather
  // than as whatever happens where two subject territories cross. Growth
  // already decided which cell belongs to which region, so this only has to
  // read `owner` back — the same tracing the subject curves get, over a mask
  // of one region instead of a union of several.
  const zoneGround = regions.map((_, i) => {
    const mine = new Uint8Array(grid * grid);
    for (let c = 0; c < owner.length; c++) if (owner[c] === i) mine[c] = 1;
    return {
      anchor: deepest(mine, grid, extent),
      loops: outline(mine, grid).map((loop) => round(loop, smooth, grid, extent)),
    };
  });

  return {
    subjects,
    zones: regions.map(({ key: k, subjects: s, population, cx, cy }, i) => ({
      key: k,
      subjects: s,
      population,
      // Where to hang the label, in user units.
      x: zoneGround[i].anchor.x,
      y: zoneGround[i].anchor.y,
      /** Radius of the largest circle that fits here; what a label has to fit in. */
      room: zoneGround[i].anchor.room,
      /** This room's own ground, as closed loops — what to fill to draw it. */
      loops: zoneGround[i].loops,
      // Where it grew from, kept for anyone wanting the seeding back.
      seed: { x: cx, y: cy },
    })),
    curves,
    extent,
    /** Which subjects the mould joined up, when one was run. */
    network: woven ? woven.network() : [],
 ...
 */
function placeSubjects(subjects, regions, extent, anchors) {
  ...
  // A subject's anchor is where it belongs by virtue of what it *is*, before
  // anybody joined it. Starting there and letting membership adjust gives the
  // map a skeleton that does not move when the population does — the
  // instability that makes this a snapshot rather than a surface. Without one,
  // the old scatter, and membership decides everything as before.
  const nodes = subjects.map((id, i) => {
    const anchor = anchors?.get(id);
    return anchor
      ? { id, x: anchor.x, y: anchor.y, ax: anchor.x, ay: anchor.y }
      : {
          id,
  ...
  for (let step = 0; step < 400; step++) {
    ...
    // Anchored subjects are drawn back toward where they belong, so shared
    // membership bends the map rather than redrawing it.
    //
    // Firmly, because the whole point is that the map holds still: at a gentle
    // pull a churning population moved subjects 182 units of a thousand, and
    // at this one it is 76 — with no cost at all to the areas, which stay
    // exact either way. The gentler value was tuned against a different
    // hierarchy and never revisited when the hierarchy changed.
    for (const node of nodes) {
      if (node.ax === undefined) continue;
      node.x += (node.ax - node.x) * 0.3;
      node.y += (node.ay - node.y) * 0.3;
    }
  }
...
/** Set a Physarum model going over the subjects, fed by who bridges them. */
function weaveBetween(subjects, regions, at, extent, grid, settings) {
  const weight = new Map();
  const shared = new Map();

  for (const r of regions) {
    for (const s of r.subjects) weight.set(s, (weight.get(s) ?? 0) + r.population);
    for (let i = 0; i < r.subjects.length; i++) {
      for (let j = i + 1; j < r.subjects.length; j++) {
        const pair = `${r.subjects[i]}+${r.subjects[j]}`;
        shared.set(pair, (shared.get(pair) ?? 0) + r.population);
  ...
  const affinity = [...shared].map(([pair, n]) => {
    const [a, b] = pair.split('+');
    return [a, b, n];
  });

  const anchors = new Map(subjects.map((s) => [s, { x: at.get(s).x, y: at.get(s).y }]));
  return weave({
    grid,
    extent,
    anchors,
...
function seedRegions(regions, at, extent, fill) {
  ...
  // The middle of everything, for a region that is nothing but a frame; see
  // `frames` in `atlas`. It belongs to no subject in particular.
  let midX = 0;
  let midY = 0;
  ...
  regions.forEach((r, i) => {
    ...
    r.ax = r.subjects.length ? x / r.subjects.length : midX;
    r.ay = r.subjects.length ? y / r.subjects.length : midY;
  ...
  for (let i = 0; i < regions.length; i++) {
    for (let j = i + 1; j < regions.length; j++) {
      // A frame's own region shares nothing with the rest as far as where it
      // goes is concerned: it only keeps clear of them, and is drawn back to
      // the middle. Pulled against every region instead, it sat in the middle
      // of them and pushed apart the ground of subjects that belonged
      // together; with no pull at all it drifted off, and the frame round the
      // group came out in two pieces. Measured over forty random groups, this
      // draws the rooms nearly as well as the same people are drawn outside
      // a group and never breaks the frame.
      const framed = !regions[i].subjects.length || !regions[j].subjects.length;
      const shared = framed ? 0 : regions[i].subjects.filter((s) => regions[j].subjects.includes(s)).length;
 ...
 * for it afterwards.
 */
function grow(regions, grid, fill, extent, woven) {
  ...
  // Where the mould has trodden, ground is cheap to claim; elsewhere it is
  // dear. Growth therefore runs along the channels rather than swelling in
  // circles — and because this only reorders the frontier, every zone still
  // stops at exactly its quota.
  const field = woven ? woven.field() : null;
  ...
  state.forEach((s, i) => {
    ...
    owner[cell] = i;
    s.filled = 1;
    pushNeighbours(s, cell, grid, owner, regions[i], extent, field);
  });
  ...
  while (placed < budget) {
    ...
    s.filled++;
    placed++;
    pushNeighbours(s, cell, grid, owner, regions[pick], extent, field);
  }
...
function pushNeighbours(state, cell, grid, owner, region, extent, field) {
  const gx = cell % grid;
  ...
  for (const [dx, dy] of [[1, 0], [-1, 0], [0, 1], [0, -1]]) {
    ...
    // discounted where the mould has laid a channel, so it follows one when
    // there is one to follow.
    const distance = (x + 0.5 - seedX) ** 2 + (y + 0.5 - seedY) ** 2;
    state.heap.push(next, field ? distance * (1 - 0.75 * field[next]) : distance);
  }
}
 ...
 * roomiest place to put text — and its depth is how much room is there.
 */
/** Ground where this subject is the only one — where its name is unambiguous. */
function soleTerritory(subject, regions, owner, grid) {
  // Ground a subject shares only with subjects that contain it is still its
  // own. Something every one of its regions also holds says nothing about
  // whose ground is whose — inside a group every room holds the group's own
  // conversation, which is what wraps them, and without this no subject in a
  // group had ground of its own and every name landed in the middle of its
  // overlaps.
  const mine = regions.filter((r) => r.subjects.includes(subject));
  const around = new Set(
    (mine[0]?.subjects ?? []).filter((s) => s !== subject && mine.every((r) => r.subjects.includes(s))),
  );
  const alone = regions.map(
    (r) => r.subjects.includes(subject) && r.subjects.every((s) => s === subject || around.has(s)),
  );
  const mask = new Uint8Array(grid * grid);
  for (let c = 0; c < owner.length; c++) {
...
function deepest(mask, grid, extent, fallback) {
  ...
  if (at === -1) return { x: 0, y: 0, room: 0 };
  return {
    x: (((at % grid) + 0.5) / grid) * extent - extent / 2,
    y: ((((at / grid) | 0) + 0.5) / grid) * extent - extent / 2,
    // How much space there is here, as the radius of the largest circle that
    // fits — which the distance transform above has already worked out and was
    // throwing away. It is what decides whether a label can be spelled out.
    room: (best * extent) / grid,
  };
}

/** Whether all the ground drawn is one piece. */
function whole(owner, grid) {
  const mask = new Uint8Array(owner.length);
  for (let c = 0; c < owner.length; c++) if (owner[c] >= 0) mask[c] = 1;
  return countComponents(mask, grid) <= 1;
}
