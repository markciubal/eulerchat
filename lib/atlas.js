...
export function atlas(zoneCounts, options = {}) {
  ...
  const at = placeSubjects(subjects, regions, extent, anchors);
  seedRegions(regions, at, extent, fill);

  // The mould weaves the channels between subjects; the quotas below still
  // decide how much ground each zone gets, so the shape changes and the areas
  // do not.
  const woven = mold ? weaveBetween(subjects, regions, at, extent, grid, mold) : null;
  ...
  // Where each zone's own label goes. The seed it grew from is not good
  // enough: growth is bounded by its neighbours, so a zone can end up nowhere
  // near where it started, and a label pinned to the seed can land outside the
  // ground it names.
  const zoneAnchors = regions.map((_, i) => {
    const mine = new Uint8Array(grid * grid);
    for (let c = 0; c < owner.length; c++) if (owner[c] === i) mine[c] = 1;
    return deepest(mine, grid, extent);
  });
