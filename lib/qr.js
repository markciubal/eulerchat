...
/**
 * How many bytes of payload a given version and level can hold.
 *
 * Worth having in public: the sensible thing to do with a link too long for a
 * QR code is to shorten the link, and that decision needs a number.
 */
export function capacity(version, ec = 'M') {
  const [ecPerBlock, blocks] = BLOCKS[version][String(ec).toUpperCase()];
  const data = blocks.reduce((sum, [count, size]) => sum + count * size, 0);
  const parity = blocks.reduce((sum, [count]) => sum + count, 0) * ecPerBlock;
  return {
    codewords: data,
    total: data + parity,
    bytes: Math.floor((data * 8 - 4 - countBits(version)) / 8),
  };
}
 ...
 *
 * Derived from the geometry rather than tabulated: the module count, minus
 * everything the function patterns take, divided by eight. That makes it an
 * independent route to a number the block table also implies, and the tests
 * insist the two agree — which is how a mistyped digit in a forty-entry table
 * gets caught, since reading the table again never catches it.
 */
export function totalCodewords(version) {
  ...
  if (centres.length) {
    ...
    // The ones centred on row or column 6 straddle a timing pattern and share
    // five modules with it, which would otherwise be subtracted twice.
    modules -= (n * n - 3) * 25;
    modules += (2 * n - 4) * 5;
  }
 ...
 *
 * `mask` pins the data mask instead of letting the scoring choose one. Any of
 * the eight produces a valid symbol, so this changes only how the code looks
 * and how easily a camera locks onto it; it exists so that a test can compare
 * one specific arrangement rather than having to agree about aesthetics first.
 *
 * @param {string} text what the code should resolve to, normally a URL
 * @param {{ec?: 'L'|'M'|'Q'|'H', mask?: number|null}} [options]
 * @returns {{size: number, version: number, ec: string, mask: number, modules: boolean[][]}}
 *   `modules[row][col]` is true where the module is dark.
 */
export function qr(text, { ec = 'M', mask: forced = null } = {}) {
  ...
  for (let v = 1; v <= MAX_VERSION; v++) {
    if (bytes.length <= capacity(v, level).bytes) {
      version = v;
      break;
    }
  }
