...
import { inner, named as spoken } from './cluster.js';
...
export const defaults = {
  ...
  lifecycle: true,
  /**
   * Whether what the server writes is worth hearing about at all: a question
   * to start a quiet chat, and everything the made-up people of a demo say.
   * Off, they are never counted and never announced — the messages still
   * arrive in the room, where a page can fold them away; see `hushSystem` in
   * `public/app.js`.
   */
  system: true,
...
function forMessage(message, subjects, prefs, muted) {
  ...
  // Written by the server rather than by a person, and muted: not news.
  if (message.machine && prefs.system === false) return null;
  ...
  // Counted without the group's own conversation, which every room inside a
  // group holds: art in a group is as broad as art, not an overlap.
  else if (inner(subjects).length > 1 || reach <= prefs.intimate) level = NOTIFY;
  ...
  // A question the server wrote is never worth interrupting anybody for, even
  // in a room of three: it is there to be found, not to call anybody over.
  if (message.machine) level = QUIET;
  void muted;
  ...
  return {
    ...
    at: message.at,
    from: message.author,
    // Who, as well as what they are called. Anybody can type any name, so a
    // name is no use for recognising somebody this person has muted; the key,
    // or failing that the id, is. Nothing new is shown by it: a notification
    // only ever goes to somebody who can read the message it is about, and
    // the message carries both.
    fromId: message.authorId ?? null,
    fromKey: message.authorKey ?? null,
    messageId: message.id,
    title: named
      ? `${message.author} mentioned you in ${spoken(subjects).join(' and ')}`
      : spoken(subjects).join(' and '),
...
function forLifecycle(event, subjects, prefs) {
  ...
  // Inside a group, the group's own conversation does not make an overlap.
  if (inner(subjects).length < 2) return null;
  ...
  return {
    ...
    at: event.at,
    title: opened
      ? `${spoken(subjects).join(' and ')} now exists`
      : `${spoken(subjects).join(' and ')} is empty`,
    body: opened
      ? event.population <= 1
